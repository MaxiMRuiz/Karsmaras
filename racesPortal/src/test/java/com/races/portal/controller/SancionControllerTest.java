...
public class SancionControllerTest {
	...
	public void listaSancionesTest() {
		try {
			...
			Mockito.when(resultados.buscarResultado(Mockito.any(), Mockito.any(), Mockito.any()))
					.thenReturn(new Resultado(1L, new Inscripcion(1L, new Piloto(),null, null), new SesionGP(), 1, 1, 1));
	...
	public void formularioEquiposTest() {
		try {
			Mockito.when(resultados.buscarResultado(Mockito.any(), Mockito.any(), Mockito.any()))
					.thenReturn(new Resultado(1L, new Inscripcion(1L, new Piloto(),null, null), new SesionGP(), 1, 1, 1));
			mockMvc.perform(get(base + "/new").sessionAttr(Constants.JWT_ATTR, JWT)
	...
	public void borrarEquipoTest() {
		try {
			...
			mockMvc.perform(delete(base + "/1").sessionAttr(Constants.JWT_ATTR, JWT)
					.sessionAttr(Constants.USER_ATTR, USER)).andExpect(status().is2xxSuccessful()).andReturn();
		} catch (Exception e) {
			fail(e.getMessage());
		}
	...
	public void postFormularioEquiposEditarTest() {
		try {
			...
			mockMvc.perform(post(base).flashAttr("sancion", new Sancion())
					.sessionAttr(Constants.JWT_ATTR, JWT).sessionAttr(Constants.USER_ATTR, USER))
					.andExpect(status().is3xxRedirection()).andReturn();
		} catch (Exception e) {
			fail(e.getMessage());
